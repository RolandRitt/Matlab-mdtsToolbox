classdef markRangeOnAxes_givenIndex_TestClass < matlab.unittest.TestCase
    %
    ...
    methods (Test)
        ...
        function testFunctionality(testCase)
            ...
            [ax] = returns.plot;
            returns.markRangeOnAxes_givenIndex(ax, [1,200, 465, 800], [100, 250, 589, returns.nDataPoints], 'r');
            

            returns1 = mdtsObject(time, data, tags, 'units', units, 'ts', ts, 'name', name, 'who', who, 'when', when, 'description', description, 'comment', comment, 'tsEvents', tsEvents, 'segments', segments);
            [ax] = returns1.plot;
            returns1.markRangeOnAxes_givenIndex(ax, [1,200, 465, 800], [100, 250, 589, returns.nDataPoints], 'r');
            [ax] = returns1.plot;
