...
\usepackage{listings}
\usepackage{color}
\usepackage[draft]{todonotes} % draft/disable
...
\begin{itemize}
	\item Web-Url: \url{https://gitlab.ia.unileoben.ac.at/tsdev/mdtstoolbox}
	\item Git-Link: \url{git@gitlab.ia.unileoben.ac.at:tsdev/mdtstoolbox.git}
\end{itemize}
...
\begin{enumerate}
	\item IAToolboxes/DOPbox: Git-Link \url{git@gitlab.ia.unileoben.ac.at:toolboxes/DOPbox.git}
	\item IAToolboxes/graphics: Git-Link \url{git@gitlab.ia.unileoben.ac.at:toolboxes/graphics.git}
	\item IAToolboxes/general: Git-Link \url{git@gitlab.ia.unileoben.ac.at:toolboxes/general.git}
	\item IAToolboxes/figureManager: Git-Link \url{git@gitlab.ia.unileoben.ac.at:toolboxes/figureManager.git}
\end{enumerate}
...
\begin{align*}
\text{obj} = \text{mdtsCoreObject}(&\text{time, }\text{data,  }\text{tags, }\text{units, }\text{ts, }\text{name, }\\&\text{who, }\text{when, }\text{description, }\text{comment, }\text{tsEvents, }\text{symbReps, }\text{segments})
\end{align*}
...
\subsection{serverData2mdtsObject}

This function loads data from the server and initializes a mdtsObject with this data. It is an interface between the data storage of a project and the mdtsToolbox. Thus, it has to be adapted or rewritten for every project.
\todo[inline]{Diese Funktion laedt derzeit ein ganz bestimmtes Projekt. Aber wie soll das konkret aussehen, wenns fertig ist? Soll diese Funktion ueberhaut irgendeine allgemeingueltige Relevanz haben? Habs im Moment so in der Doku gelassen, das muss aber noch angepasst werden.}

\subsubsection{Function Signature:}

\begin{center}
	returnObject = serverData2mdtsObject
\end{center}

\subsubsection{Purpose}

Get data from server and store it in a mdtsObject.
...
\begin{longtabu} to \textwidth {|c|X|}
	...
	\textbf{Parameter} & \textbf{Description} \\ \hline
	\endhead
	returnObject & The mdtsObject containing the loaded data \\ \hline
\end{longtabu}

\subsubsection{Description}

Access a sever, get the data from the server and pass it to a mdtsObject. Return the generated object.
