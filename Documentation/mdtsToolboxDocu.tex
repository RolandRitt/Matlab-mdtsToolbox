...
\section{Symbolic Representation -Classes and Methods}
...
\subsection{A = allcomb(varargin)}
	A function to get all permutation of mutliple input arrays. This function is taken from \url{https://de.mathworks.com/matlabcentral/fileexchange/10064-allcomb-varargin}. For documentation see function documentation.
...
\subsection{FigureManager\_hierarchicalSymRep}
	A subclass of \textit{FigureManager} (see FigureManager toolbox). This is used to visualize hierarchical symbolic analysis. This is used in the function \textit{visuHierarchicalSymRep}-see \ref{ssec:visuHierarchicalSymRep}. This function is under development.
	
\subsection{bTrue = isHierarchicalSymRep(hierarchicalSymbolicData)}
	A function which checks, if the given cell array \textit{hierarchicalSymbolicData} is a cell array of \textit{SymbRepObject}'s. Returns true if it is the case.

\subsection{gObjArr = plotSymRepObjectOnAllAxes(axes\_in, SymbRepObj, xTime, varargin)}

	\subsubsection{Function Signature:}
	
		\begin{center}
			gObjArr = plotSymRepObjectOnAllAxes(axes\_in, SymbRepObj, xTime, varargin)
		\end{center}
	
	\subsubsection{Purpose}
	
		This function plots an instance of a SymbRepObject onto the given axes (using semi-transparent patches).
	...
	\subsubsection{Input Parameters:}
		...
		\begin{longtabu} to \textwidth {|c|X|}
			...
			\textbf{Parameter} & \textbf{Description} \\ \hline
			\endhead
			axes\_in & the axes, on which the symbolic representation should be plotted. \\ \hline
			SymbRepObj & a single instance of a SymbRepObject, which should be visualized on all given axes (axes\_in); \\ \hline
			xTime & The abscissae values the symbolic representation refers to. This is used for plotting and has to be consistent with the axes\_in \\ \hline
			plotSymbolName & (optional key-value, default: false)a boolean indicating if the Symbol name should be shown in the plot;\\ \hline
			plotSymbolDuration & (optional key-value, default: false )a boolean indicating if the Symbol duration should be shown in the plot;\\ \hline
			plotSymbolNameMinLength & (optional key-value, default: 0) an integer, symbols with a length less than  this value are not annotated with symbol name and/or duration;\\ \hline
			colorDismiss & (optional key-value, default: no color) a color string or color triplet which should not be in the colors used for shading the plots.\\ \hline
			additional inputs &  additional key-value pairs are forwarded to the matlab fill function.\\ \hline
		\end{longtabu}
	...
	\subsubsection{Return Parameters:}
		...
		\begin{longtabu} to \textwidth {|c|X|}
			...
			\textbf{Parameter} & \textbf{Description} \\ \hline
			\endhead
			gObjArr  &  an array containing the hggroups for all axes, containing the symbolic representation; \\ \hline
		\end{longtabu}
	
	
\subsection{gObjArr = plotSymRepObjectOnAxes(axes\_in, SymbRepObjs, xTime, varargin)}

	\subsubsection{Function Signature:}
	
		\begin{center}
			gObjArr = plotSymRepObjectOnAxes(axes\_in, SymbRepObjs, xTime, varargin)
		\end{center}
	
	\subsubsection{Purpose}
	
		This function plots multiple instances of SymbRepObjects onto multiple axes. There is a SymbRepObject for each axes.
	...
	\subsubsection{Input Parameters:}
		...
		\begin{longtabu} to \textwidth {|c|X|}
			...
			\textbf{Parameter} & \textbf{Description} \\ \hline
			\endhead
			axes\_in & the axes, on which the symbolic representations should be plotted. \\ \hline
			SymbRepObjs & a cellarry which contains multiple instances of a  SymbRepObject. The SymbRepObjs{i} should be visualized in \\ \hline
			xTime & The abscissae values the symbolic representation refers to. This is used for plotting and has to be consistent with the axes\_in \\ \hline
			plotSymbolName & (optional key-value, default: false)a boolean indicating if the Symbol name should be shown in the plot;\\ \hline
			plotSymbolDuration & (optional key-value, default: false )a boolean indicating if the Symbol duration should be shown in the plot;\\ \hline
			plotSymbolNameMinLength & (optional key-value, default: 0) an integer, symbols with a length less than  this value are not annotated with symbol name and/or duration;\\ \hline
...
\subsection{symbRepChannel}
	...
	\subsubsection{Purpose}
		...
		Generate the symbolic representation (instance of SymbRepObject) of a given channel of an mdtsObject.
...
\subsection{visuHierachicalSymRep}
\label{ssec:visuHierarchicalSymRep}
	
	
	\subsubsection{Function Signature:}
		
		\begin{center}
			[Ax, fM, figH] = visuHierachicalSymRep(mdtsObj, HierarchicalSymRep, varargin)
		\end{center}
	
	\subsubsection{Purpose}
	
		Function to plot hierarchical symbolic Representation for a given mdts-object. \textit{This function is under development}
	...
	\subsubsection{Input Parameters:}
		...
		\begin{longtabu} to \textwidth {|c|X|}
			...
			\textbf{Parameter} & \textbf{Description} \\ \hline
			\endhead
			mdtsObj & an mdtsObject which is used to generate the plot and is the base for the hierarchical symbolic time series \\ \hline
			HierarchicalSymRep & A cell which contains an instance of a SymbRepObject for each level and is used to visualize\\ \hline
		\end{longtabu}
	...
	\subsubsection{Return Parameters:}
		...
		\begin{longtabu} to \textwidth {|c|X|}
			...
			\textbf{Parameter} & \textbf{Description} \\ \hline
			\endhead
			Ax  &  The handles to the axes\\ \hline
			fM  &  The handle to the used FigureManager instance\\ \hline
			figH  &  The handle to generated figure\\ \hline
		\end{longtabu}
		
\subsection{visuHierarchicalCompoundingOverview}
\label{ssec:visuHierarchicalSymRep}

	\subsubsection{Function Signature:}

		\begin{center}
			figureH = visuHierarchicalCompoundingOverview(compressionData, imageMatrix,varargin)
		\end{center}

	\subsubsection{Purpose}
	
		Function to plot hierarchical symbolic Representation overview. This function esembles different kpi's such as compression ratio and an image representation of all levels \textit{This function is under development}
	...
	\subsubsection{Input Parameters:}
		...
		\begin{longtabu} to \textwidth {|c|X|}
			...
			\textbf{Parameter} & \textbf{Description} \\ \hline
			\endhead
			compressionData & A structure containing the hierarchical KPI's. This is returned by the class function \textit{SymbRepObject.genHierarchicalCompounding} and \textit{SymbRepObject.genHierarchicalCompounding2} \\ \hline
			imageMatrix & A matrix which represents the hierarchical compoounding.  This is returned by the class function \textit{SymbRepObject.genHierarchicalCompounding} and \textit{SymbRepObject.genHierarchicalCompounding2}\\ \hline
		\end{longtabu}
	...
	\subsubsection{Return Parameters:}
		...
		\begin{longtabu} to \textwidth {|c|X|}
			...
			\textbf{Parameter} & \textbf{Description} \\ \hline
			\endhead
			figureH  &  The handle to generated figure\\ \hline
		\end{longtabu}
