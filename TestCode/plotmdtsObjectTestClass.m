classdef plotmdtsObjectTestClass < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testRelativeTimeStampsMs(testCase)
            ...
            for i = 1 : nX
                ...
                time(i) = datenum(datetime(0, 0,0, 0, 0, 0 + i * 0.1 * seconds(ts)));
            ...
            returns = mdtsObject(time, data, tags, 'units', units, 'ts', ts, 'name', name, 'who', who, 'when', when, 'description', description, 'comment', comment, 'absoluteTS', false, 'tsEvents', tsEvents);
